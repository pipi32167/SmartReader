...
SmartReader 是一款基于 Chrome Extension Manifest V3 的浏览器扩展，用于总结网页内容并通过 AI API 回答用户提问。它能够将网页内容提取为 Markdown，发送到兼容 OpenAI 的 API 端点，并将流式响应展示在侧边面板中。扩展同时支持对 PDF 文件的文本提取与摘要。
...
- **Tesseract.js** — OCR 识别扫描型 PDF（运行在 Offscreen Document 中，语言包从 CDN 加载）
...
src/
  background/
    service-worker.ts    # Service Worker：中央协调器、AI 流式请求、DB 代理、PDF 处理
  content/
  ...
  popup/
    popup.html           # 弹出窗口 UI
    popup.ts             # 弹出窗口逻辑：提示词列表、自定义输入、打开侧边面板
    popup.css
  sidepanel/
    sidepanel.html       # 侧边面板 UI
    sidepanel.ts         # 侧边面板逻辑：接收流式消息、渲染 Markdown
    sidepanel.css
  options/
    options.html         # 设置页面 UI
    options.ts           # 设置页面逻辑：API 配置与提示词管理
    options.css
  shared/
    types.ts             # TypeScript 接口与 MessageType 常量
    constants.ts         # 默认 API 配置、默认提示词、MAX_CONTENT_LENGTH
...
2. **Service Worker** 注入或与 **Content Script** 通信，获取页面内容。
...
5. **Side Panel** 使用 `marked` 渲染内容，并提供复制/中止控制。

### PDF 处理流程
当检测到当前标签页为 PDF 时（检测逻辑：URL 模式匹配 + 可选的 HEAD 请求验证 `Content-Type: application/pdf`）：
1. Service Worker 下载 PDF 并转为 Base64。
2. **PDF → Markdown**：将 PDF 字节发送到 Offscreen Document，由 `pdfjs-dist` 逐页提取文本；若某页文本极少（< 50 字符），则将该页渲染到 Canvas 并用 `Tesseract.js`（`chi_sim+eng`）进行 OCR。提取的文本经 `buildPdfHtml` 组装为语义化 HTML，再通过 `htmlToMarkdown` 转为 Markdown。
3. **AI Prompt**：`executePrompt` 会将转换后的 Markdown 作为 `${html}` 变量填入提示词，而非仅显示「这是一份 PDF」占位符。
4. **直接查看**：`SHOW_PAGE_MARKDOWN`（popup「查看 Markdown」）会直接显示转换后的 Markdown。
5. 优先尝试以 OpenAI `file` 类型的 content part 直接上传 PDF。
6. 若 API 不支持文件上传，fallback 时提示词中已包含 Markdown 内容，无需再次提取。
...
- **命令**：`npm test`
- **当前覆盖范围**：测试主要聚焦于 `src/shared/html-to-markdown.ts`，涵盖标题、链接、图片、列表、表格、媒体元素、隐藏元素移除以及嵌套 Shadow DOM。
...
完整的消息类型列表：
- `GET_PAGE_CONTENT` — Content Script 获取页面内容
...
- `EXTRACT_PDF_TEXT` — PDF 纯文本提取（AI fallback 用）
- `CONVERT_PDF_TO_MARKDOWN` — PDF → HTML → Markdown 完整转换
