...
- **pdfjs-dist** — PDF 文本提取（运行在 Offscreen Document 中）
...
Manifest V3 的 Service Worker 无法执行 WASM，也无法访问 OPFS。因此扩展使用 **Offscreen Document**（`src/offscreen/offscreen.ts`）来托管 `sql.js` 和 `pdfjs-dist`，并通过 OPFS 读写 SQLite 数据库。Service Worker 通过 `chrome.runtime.sendMessage` 将所有数据库操作代理到该文档。
...
2. 优先尝试以 OpenAI `file` 类型的 content part 直接上传。
3. 若 API 不支持文件上传，则向 Offscreen Document 发送 `EXTRACT_PDF_TEXT`，使用 `pdfjs-dist` 提取文本后作为普通文本提示词重试。
